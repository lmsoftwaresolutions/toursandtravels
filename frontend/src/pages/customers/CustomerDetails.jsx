...
export default function CustomerDetails() {
  ...
  useEffect(() => {
    api.get(`/customers/${id}/trips`).then(res => {
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="bg-white p-4 rounded shadow space-y-3 w-full max-w-3xl">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          ...
          <p><b>Amount Paid:</b> ₹{(
            (customer.total_billed ?? 0) - (customer.pending_balance ?? 0)
          ).toFixed(2)}</p>
          <p><b>Balance Pending:</b> <span className="text-red-600 font-semibold">₹{customer.pending_balance?.toFixed(2)}</span></p>
        </div>
      </div>
              ...
              {trips.length === 0 ? (
                ...
                trips.map(t => (
                  <tr key={t.id} className="border-t">
                    <td className="p-2 font-semibold text-blue-600">{t.invoice_number || "N/A"}</td>
                    ...
                    <td className="p-2">₹{(t.total_charged ?? 0).toFixed(2)}</td>
                    <td className="p-2">₹{(t.amount_received ?? 0).toFixed(2)}</td>
                    <td className="p-2 text-red-700">₹{(t.pending_amount ?? 0).toFixed(2)}</td>
                  </tr>
                ))
              )}
            </tbody>
